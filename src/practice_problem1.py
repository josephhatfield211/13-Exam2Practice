...
class Box(object):
    ...
    def append_string(self, additional_contents):
        ...
        for k in range(self.volume):
            if len(self.contents)<self.volume:
                ...
                if len(additional_contents)-counter>0:
                    for k in range(len(additional_contents)-counter):
                        s = s + additional_contents[counter+k]
                    counter = len(additional_contents)
    ...
    def shrink(self, new_volume):
        ...
        for k in range(new_volume):
            if k<len(content):
                self.contents += content[k]
        for k in range(len(content)-self.volume):
            if self.volume+k<len(content):
                s = s + content[self.volume+k]
        return s
    ...
    def combined_box(self, other_box):
        ...
        content = self.contents
        volume = self.volume
        box = Box(content+other_box.contents,volume+other_box.volume)
        return box
